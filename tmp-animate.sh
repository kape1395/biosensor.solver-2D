...
for f in `ls -1 tmp-out10/concentrations-*`; do echo -n "$f "; head -n 1 $f; done | awk '
BEGIN {
  printf("set term gif small animate transparent opt delay 10 size 400,400 x000000\n");
  printf("set output \"tmp-out10-animate.gif\"\n");
  printf("set pm3d map\n");
...
}
{
  ...
  printf("set cbrange[0:0.100];set title \"P\";     splot [0:40][0:60][0:100] \"%s\" using 1:2:4 notitle\n", $1);
...
' > tmp-out10-animate.gpl
gnuplot tmp-out10-animate.gpl
